...
"${PSQL[@]}" -f "$HERE/migrations/0008_outbound_ops_governance.up.sql"
"${PSQL[@]}" -f "$HERE/migrations/0009_rls_outbound_ops_governance.up.sql"
"${PSQL[@]}" -f "$HERE/migrations/0010_knowledge_base.up.sql"
"${PSQL[@]}" -f "$HERE/migrations/0011_rls_knowledge_base.up.sql"
...
echo "==> RLS + WORM + partition + karışık-politika testi (Outbound/Operasyon/Yönetişim)"
"${PSQL[@]}" -f "$HERE/tests/outbound_ops_governance_isolation.sql"

echo "==> RLS + namespace izolasyon testi (Bilgi Tabanı / Vector Store)"
"${PSQL[@]}" -f "$HERE/tests/knowledge_base_isolation.sql"

echo "==> Temizlik (down)"
"${PSQL[@]}" -f "$HERE/migrations/0011_rls_knowledge_base.down.sql"
"${PSQL[@]}" -f "$HERE/migrations/0010_knowledge_base.down.sql"
"${PSQL[@]}" -f "$HERE/migrations/0009_rls_outbound_ops_governance.down.sql"
"${PSQL[@]}" -f "$HERE/migrations/0008_outbound_ops_governance.down.sql"
